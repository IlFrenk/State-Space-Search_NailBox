...
def breadth_first_tree_search(problem):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            print("Max depth reached: "+str(maxDepth)+" Nodes expanded: "+str(counter))
            return node
        frontier.extend(node.expand(problem))
        counter+=len(frontier)-temp
        print_line(str(counter),str(maxDepth))
    return None
...
class Fontana(Problem):
    ...
    def actions(self, state):
        ...
        possible_actions = ['FillA', 'FillB', 'EmptyA', 'EmptyB', 'PutAtoB', 'PutBtoA']

        if state["a"] == 3:
            possible_actions.remove('FillA')
        if state["b"] == 5:
            possible_actions.remove('FillB')
        if state["a"] == 0:
            possible_actions.remove('EmptyA')
        if state["b"] == 0:
            possible_actions.remove('EmptyB')
        if state["a"] == 0 or state["b"] == 5:
            possible_actions.remove('PutAtoB')
        if state["b"] == 0 or state["a"] == 3:
    ...
    def result(self, state, action):
        ...
        new_state = {"a":state["a"],"b":state["b"]}
        #new_state=state WRONG (creates another reference to the object, does not copy it)
        #OR new_state=dict(state)
        #OR new_state=state.copy()
        if action == "EmptyA":
                new_state["a"]=0
        elif action == "EmptyB":
                new_state["b"]=0
                ...
                if(state["a"] <= (5-state["b"])):
                        new_state["b"]=state["b"]+state["a"]
                        new_state["a"] = 0
                else:
                        new_state["a"] = state["a"] - (5-state["b"])
                        new_state["b"] = 5
        elif action == "PutBtoA":
                if(state["b"] <= (3-state["a"])):
                        new_state["a"]=state["a"]+state["b"]
                        new_state["b"] = 0
                else:
                        new_state["b"] = state["b"] - (3-state["a"])
                        new_state["a"] = 3
        return new_state
